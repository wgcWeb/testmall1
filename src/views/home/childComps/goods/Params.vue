  ...
  methods: {
    ...
    addParams() {
      this.$refs.addFormRef.validate((valid) => {
        if (!valid) return
        getAddParams(this.cateId, this.addForm.attr_name, this.activeName).then(
          (res) => {
            // console.log(res);
    ...
    showEditDialog(attr_id) {
      getFindParams(this.cateId, attr_id, this.activeName).then((res) => {
        // console.log(res);
        if (res.meta.status !== 200) {
    ...
    removeParams(attr_id) {
      this.$confirm('此操作将永久删除该参数, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          //   message: '删除成功!',
          // })
          getDelParams(this.cateId, attr_id).then((res) => {
            // console.log(res);
            if (res.meta.status !== 200) {
